...
class ClimateLogSerializer(serializers.ModelSerializer):
    ...
    recorder = serializers.CharField(max_length=40, allow_blank=True, required=False)
    reviewer = serializers.CharField(max_length=40, allow_blank=True, required=False)
    unsigned = serializers.SerializerMethodField()
    ...
    class Meta:
        ...
        read_only_fields = ("id", "zoneCode", "greenhouseName", "unsigned", "created_at")

    def get_unsigned(self, obj):
        return is_unsigned(obj.recorder, obj.reviewer)
    ...
    def validate(self, attrs):
        # 新建与更新共用同一套双签校验：历史缺签行可读，但更新必须补齐双签。
        recorder = attrs.get("recorder")
        reviewer = attrs.get("reviewer")
        if self.instance is not None:
            if recorder is None:
                recorder = self.instance.recorder
            if reviewer is None:
                reviewer = self.instance.reviewer
        attrs["recorder"], attrs["reviewer"] = validate_dual_sign(recorder, reviewer)
        return attrs
...
class IrrigationCycleSerializer(serializers.ModelSerializer):
    ...
    def validate(self, attrs):
        # 仅新建拦截：分区存在缺签气候记录时禁止新建轮灌（与缺签判定共用 signing 口径）。
        if self.instance is None:
            zone = attrs.get("zone")
            if zone is not None and is_zone_irrigation_blocked(zone.pk):
                raise serializers.ValidationError(
